...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        """ Apply the encoder to source sentences to obtain encoder hidden states.
            ...
            Note that batch size is the nums of sentences.
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        y_w=self.model_embeddings.target(target_padded)
        for sp in torch.split(y_w,1):
            y_part=torch.squeeze(sp,0)
            y_bart=torch.cat((y_part,o_prev),1)
            dec_state,o_t,e_t=self.step(y_bart,dec_state,enc_hiddens,enc_hiddens_proj,enc_masks)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        alhpa_t=torch.nn.functional.softmax(e_t,1)
        alhpa_t=torch.unsqueeze(alhpa_t,1)
        a_t=torch.squeeze(torch.bmm(alhpa_t,enc_hiddens),1)
        u_t=torch.cat((a_t,dec_hidden),1)
        v_t=self.combined_output_projection(u_t)
        O_t=self.dropout(torch.nn.Tanh()(v_t))
